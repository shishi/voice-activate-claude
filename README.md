...
Linux では nix flake で開発環境(Python + uv)が入る:
`nix develop -c uv run pytest`(direnv 利用時は `direnv allow`)。
CI(GitHub Actions)も同じ devShell で `uv sync --locked` → `pytest` を実行する。
