...
## いまの一番の課題(root cause 確定・修正済み・実機検証待ち)

**注入が chat/Cowork トグルの取得で失敗する問題 → root cause 確定(2026-07-11)。**
...
- **次にやること**: shishi が実機で `git pull ; uv run python -m vac.check inject "診断テスト"` → チャットモードの新規チャットに「診断テスト」が入って送信されるか確認。失敗時は `resolve failed ... dumping candidates` のログを見る(46ece89 の診断が恒久保険として残っている)。
...
- **注入の実機検証**(上記・最優先。root cause 修正済み、実機で通るかの確認待ち)。
