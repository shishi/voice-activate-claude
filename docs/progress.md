...
## 速度について(root cause 確定・高速化実装済み・実機検証待ち)
...
- Codex native ゲート6周で clean。途中で直した点: 最小権限 OpenProcess(昇格プロセス対応)、カスタム --exe の認識、トレイ格納窓の stale 扱い、UIA 未準備時の wrap 失敗許容と二重起動防止。
- 期待値: 約3秒(検索~0.1s + 前面化~0.5s + resolve 4回×0.2s + settle 1.2s)。**実機未検証**。
...
- **高速化の実機検証**(最優先。実装・レビュー済み、期待約3秒。`git pull ; uv run python -m vac.check inject "診断テスト"` を2回=Code タブ開始とコールドスタートで確認)。
